...
class HTTP_Header extends ArrayObject {
	 ...
	 *
	 *     $header_object = new HTTP_Header(array('x-powered-by' => 'Core 3.1.x', 'expires' => '...'));
	 *
	...
	public function send_headers(HTTP_Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		...
		{
			$processed_headers[] = 'Content-Type: '.Core::$content_type.
				'; charset='.Core::$charset;
		}

		if (Core::$expose AND ! isset($headers['x-powered-by']))
		{
			$processed_headers[] = 'X-Powered-By: Core Framework '.
				Core::VERSION.' ('.Core::CODENAME.')';
		}
